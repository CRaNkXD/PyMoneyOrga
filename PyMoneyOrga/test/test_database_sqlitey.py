...
class Test_account(unittest.TestCase):
    ...
    def test_add_acc_second(self):
        """ tests if a second account can be set by checking table columns of the first and the second entry """
        self.database.add_acc("Daniel",100)
    ...
    def test_get_acc_none(self):
        """ tests if get acc returns a dict {acc_name: None} if the specified account does not exist """
        acc = self.database.get_acc("Daniel")
    ...
    def test_add_transactions_first(self):
        ...
        self.database.add_acc("Daniel",100)
        self.database.add_transaction("Daniel",100,50)
    ...
    def test_add_transactions_second(self):
        """ tests if a second transaction can be set by checking table columns of the first and the second entry """
        self.database.add_acc("Daniel",100)
        self.database.add_transaction("Daniel",100,50)
        self.database.add_transaction("Daniel",200,100)
        transactions = self.database.get_all_transaction("Daniel")
        self.assertEqual(transactions[0].amount,100)
        self.assertEqual(transactions[0].new_balance,50)
