class Money(object):
    ...
    def __sub__(self, other):
        if isinstance(other, self.__class__):
            if self._currency != other.currency:
                print(f"Cannot substract money when the currency does not match ({self._currency} != {money.currency}")
            amount = self._amount - other.amount
            return Money(amount, self._currency)
        elif isinstance(other, int):
            amount = self._amount - other
    ...
    def __mul__(self, other):
        if isinstance(other, self.__class__):
            if self._currency != other.currency:
                print(f"Cannot multiply money when the currency does not match ({self._currency} != {money.currency}")
            amount = self._amount * other.amount
            return Money(amount, self._currency)
        elif isinstance(other, int):
            amount = self._amount * other
    ...
    def __truediv__(self, other):
        if isinstance(other, self.__class__):
            if self._currency != other.currency:
                print(f"Cannot divide money when the currency does not match ({self._currency} != {money.currency}")
            amount = self._amount // other.amount
            return Money(amount, self._currency)
        elif isinstance(other, int):
            amount = self._amount // other
    ...
    def convert_to(cls, money: 'Money', new_currency):
        if new_currency not in cls._VALID_CURRENCIES:
            print(f"The currency {new_currency} is not valid or not implemented!")
        euro = money.amount // cls._VALID_CURRENCIES[money.currency]
        return cls(euro * cls._VALID_CURRENCIES[new_currency], new_currency)
